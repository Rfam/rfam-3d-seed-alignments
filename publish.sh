#!/bin/bash

#SBATCH --time=30:00                     # walltime
#SBATCH --ntasks=1                       # number of tasks
#SBATCH --cpus-per-task=1                # number of CPUs Per Task i.e if your code is multi-threaded
#SBATCH --nodes=1                        # number of nodes
#SBATCH -p datamover                     # partition(s)
#SBATCH --mem=10G                        # memory per node
#SBATCH -J "rfam_3d_publish"             # job name
#SBATCH -o "rfam_3d_publish.out"         # job output file
#SBATCH -e "rfam_3d_publish.err"         # job error file
#SBATCH --mail-user=bsweeney@ebi.ac.uk   # email address to message
#SBATCH --mail-type=BEGIN
#SBATCH --mail-type=END
#SBATCH --mail-type=FAIL

set -euo pipefail
IFS=$'\n\t'

# This seems to speed up the submission of jobs and thus the overall runtime of
# pipelines.
export NXF_OPTS='-Dnxf.pool.type=sync -Dnxf.pool.maxThreads=10000'

cp results/report/report.txt $RFAM_FTP/.preview/3d/
cp results/alignments/*.sto $RFAM_FTP/.preview/3d/alignments/
